...
exports.paymentsCreateIntent = paymentsCreateIntent;
exports.paymentsProcess = paymentsProcess;
exports.ordersStatus = ordersStatus;
exports.shippingRates = shippingRates;
exports.shippingLabel = shippingLabel;
exports.shippingTrack = shippingTrack;
exports.notificationsSend = notificationsSend;
...
const handleCORS = (methods = 'GET, POST, OPTIONS') => ({
    status: 200,
    headers: {
...
async function pricingCalculate(request, context) {
    if (request.method === 'OPTIONS')
        return handleCORS('POST, OPTIONS');
    try {
        ...
        const pricingData = {
            ...
            orderId,
            material,
            volume: (volume === null || volume === void 0 ? void 0 : volume.toString()) || '45.2',
...
async function ordersCreate(request, context) {
    if (request.method === 'OPTIONS')
        return handleCORS('POST, OPTIONS');
    try {
        const body = await request.json();
...
async function paymentsCreateIntent(request, context) {
    if (request.method === 'OPTIONS')
        return handleCORS('POST, OPTIONS');
    try {
        const body = await request.json();
...
async function paymentsProcess(request, context) {
    if (request.method === 'OPTIONS')
        return handleCORS('POST, OPTIONS');
    try {
        const body = await request.json();
...
async function ordersStatus(request, context) {
    if (request.method === 'OPTIONS')
        return handleCORS('GET, POST, PUT, OPTIONS');
    try {
        ...
        if (request.method === 'PUT') {
            ...
            const currentStep = statusSteps[orderStatus === null || orderStatus === void 0 ? void 0 : orderStatus.status] || 0;
            const trackingSteps = [
                { step: 'Order Received', completed: currentStep >= 0, timestamp: (orderStatus === null || orderStatus === void 0 ? void 0 : orderStatus.created) || new Date(Date.now() - 3600000).toISOString() },
            ...
            const statusResponse = {
                orderId,
                status: (orderStatus === null || orderStatus === void 0 ? void 0 : orderStatus.status) || 'in_production',
...
async function shippingRates(request, context) {
    if (request.method === 'OPTIONS')
        return handleCORS('POST, OPTIONS');
    try {
        const body = await request.json();
        const { toAddress, items } = body;
        context.log(`🚚 Getting shipping rates for ${toAddress === null || toAddress === void 0 ? void 0 : toAddress.city}, ${toAddress === null || toAddress === void 0 ? void 0 : toAddress.state}`);
        // Initialize shipping service
        const shipping = shipping_1.default.getInstance();
...
async function shippingLabel(request, context) {
    if (request.method === 'OPTIONS')
        return handleCORS('POST, OPTIONS');
    try {
        const body = await request.json();
        ...
        try {
            ...
            if (orderInfo === null || orderInfo === void 0 ? void 0 : orderInfo.customerEmail) {
        ...
        return successResponse({
            ...label,
            shippingId: (savedShipping === null || savedShipping === void 0 ? void 0 : savedShipping.shippingId) || shippingData.shippingId,
            saved: !!savedShipping
        });
    }
    catch (error) {
...
async function shippingTrack(request, context) {
    if (request.method === 'OPTIONS')
        return handleCORS('GET, POST, OPTIONS');
    try {
        const url = new URL(request.url);
...
async function notificationsSend(request, context) {
    var _a, _b;
    if (request.method === 'OPTIONS')
        return handleCORS('GET, POST, OPTIONS');
    try {
...
functions_1.app.http('shipping-rates', {
    methods: ['POST', 'OPTIONS'],
    authLevel: 'anonymous',
    handler: shippingRates
});
functions_1.app.http('shipping-label', {
    methods: ['POST', 'OPTIONS'],
    authLevel: 'anonymous',
    handler: shippingLabel
});
functions_1.app.http('shipping-track', {
